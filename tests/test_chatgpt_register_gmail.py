...
class RegistrationEngineGmailFlowTests(unittest.TestCase):
    ...
    def test_normalize_auth_page_type_supports_hyphenated_values(self):
        engine = self._make_engine()

        self.assertEqual(engine._normalize_auth_page_type("about-you"), "about_you")
        self.assertEqual(engine._normalize_auth_page_type("add-phone"), "add_phone")

    def test_create_user_account_retries_invalid_auth_step_after_navigation_repair(self):
        engine = self._make_engine()
        engine._device_id = "device-fixed"
        engine.session = mock.Mock()
        first_response = mock.Mock(status_code=400, text='{"error":{"code":"invalid_auth_step"}}')
        retry_response = mock.Mock(status_code=200, text="{}")
        repair_nav_response = mock.Mock(status_code=200, url="https://auth.openai.com/about-you")
        engine.session.post.side_effect = [first_response, retry_response]
        engine.session.get.return_value = repair_nav_response

        with mock.patch.object(engine, "_build_json_headers", return_value={"content-type": "application/json"}), \
            mock.patch.object(engine, "_build_navigation_headers", return_value={"accept": "text/html"}), \
            mock.patch.object(engine, "_check_sentinel", side_effect=["sentinel-1", "sentinel-2"]), \
            mock.patch.object(engine, "_resolve_post_otp_continue_url", return_value="https://auth.openai.com/about-you"), \
            mock.patch("platforms.chatgpt.refresh_token_registration_engine.generate_random_user_info", return_value={"name": "Demo", "birthdate": "1990-01-01"}), \
            mock.patch("platforms.chatgpt.refresh_token_registration_engine.time.sleep"):
            ok = engine._create_user_account()

        self.assertTrue(ok)
        self.assertEqual(engine.session.post.call_count, 2)
        self.assertEqual(engine.session.get.call_count, 1)

    def test_create_user_account_treats_already_exists_as_success(self):
        engine = self._make_engine()
        engine._device_id = "device-fixed"
        engine.session = mock.Mock()
        engine.session.post.return_value = mock.Mock(
            status_code=400,
            text='{"error":{"code":"already_exists"}}',
        )

        with mock.patch.object(engine, "_build_json_headers", return_value={"content-type": "application/json"}), \
            mock.patch.object(engine, "_check_sentinel", return_value="sentinel-1"), \
            mock.patch("platforms.chatgpt.refresh_token_registration_engine.generate_random_user_info", return_value={"name": "Demo", "birthdate": "1990-01-01"}):
            ok = engine._create_user_account()

        self.assertTrue(ok)

    def test_create_user_account_stops_retry_when_repair_redirects_to_add_phone(self):
        engine = self._make_engine()
        engine._device_id = "device-fixed"
        engine.session = mock.Mock()
        first_response = mock.Mock(status_code=400, text='{"error":{"code":"invalid_auth_step"}}')
        repair_nav_response = mock.Mock(status_code=200, url="https://auth.openai.com/add-phone")
        engine.session.post.return_value = first_response
        engine.session.get.return_value = repair_nav_response
        ...
        self.assertFalse(ok)
        self.assertEqual(engine._auth_block_reason, "add_phone")
        self.assertEqual(engine.session.post.call_count, 1)
    ...
    def test_resolve_oauth_callback_url_handles_organization_select_redirect(self):
        ...
        engine.session.cookies.get_dict.return_value = {
            "oai-client-auth-session": self._encode_cookie_payload(cookie_payload)
        }
